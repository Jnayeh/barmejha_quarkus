...
import org.barmejha.repositories.users.AdminRepository;
...
public class AdminService implements IEntityService<Admin, UserDTO> {
  ...
  @Override
  @WithTransaction
  public Uni<Response> create(HttpHeaders headers, Admin entity) {
    log.warn("Creating user: {}", entity);

    return adminRepository.persist(entity).map(this::toDTO).map(ServiceUtils::createdResponse);
  }

  @Override
  @WithTransaction
  public Uni<Response> update(HttpHeaders headers, Long id, Admin updatedEntity) {
    return adminRepository.findById(id).onItem().transform(found -> {
      ...
      return found;
    }).flatMap(adminRepository::persist).map(this::toDTO).map(ServiceUtils::okResponse);
  }


  @WithTransaction
  public Uni<Response> changePass(HttpHeaders headers, Long id, Admin updatedEntity) {
    return adminRepository.findById(id).onItem().transform(found -> {
  ...
  public Uni<Response> delete(HttpHeaders headers, Long id) {
    return adminRepository.deleteById(id).map(isDeleted -> {
  ...
  public Uni<Long> count(HttpHeaders headers, QueryRequest request) {
    return adminRepository.countByQuery(request);
  }
